...
/// @title Mead-BNB-LP Staking pool
/// @notice The Mead-BNB-LP staking pool allows investors to deposit Mead-BNB-LP tokens. The investor recieves each block a certain reward in mead tokens. This blockReward can be updated by an account with UPDATER_ROLE.
...
/// The Mead-BNB-LP staking pool uses EIP-1973 to for scalable rewards
contract MeadBNBFarm is
    Initializable,
    ...
    PausableUpgradeable
{
    ...
    function initialize(
        ...
        roundMask = 1;
        MeadToken = IMead(_meadTokenAddress);
    ...
    /// @notice Stakes the desired amount of Mead-BNB-LP tokens into the pool
    /// @dev Executes claimRewards before the staking to get a clean state for the roundMask and the rewards
    /// @param amount The desired staking amount for an investor in Mead-BNB-LP tokens
    ...
    /// @notice Unstakes the desired amount of Mead-BNB-LP tokens from the pool
    /// @dev Executes claimRewards before the unstaking to get a clean state for the roundMask and the rewards
    /// @param amount The desired amount to unstake for an investor in Mead-BNB-LP tokens
    ...
    /// @notice Converts the supplied amount of ETH into Mead-BNB-LP tokens, then stakes it into the pool
    /// @dev Executes claimRewards before the staking to get a clean state for the roundMask and the rewards
    function stakeFromEth() external payable nonReentrant whenNotPaused {
        ...
        (uint256 amount, uint256 unusedEth, uint256 unusedTokens) = DEX
            .convertEthToTokenLP{value: msg.value}(address(MeadToken));
        ...
        IERC20Upgradeable(address(MeadToken)).transfer(
            msg.sender,
            unusedTokens
        );
    ...
    /// @notice Unstakes the desired amount of Mead-BNB-LP tokens from the pool, then converts it into ETH and sends it back to the caller
    /// @dev Executes claimRewards before the unstaking to get a clean state for the roundMask and the rewards
    /// @param amount The desired amount to unstake for an investor in Mead-BNB-LP tokens
    ...
    /// @notice Converts the specified amount of the specified token into Mead-BNB-LP tokens, then stakes it into the pool
    /// @dev Executes claimRewards before the staking to get a clean state for the roundMask and the rewards
    /// @param token the address of the token to be converted into Mead-BNB-LP tokens
    /// @param tokenAmount the amount of tokens to be converted into Mead-BNB-LP tokens
        ...
        whenNotPaused
    {
        ...
        (uint256 amount, uint256 unusedEth, uint256 unusedTokens) = DEX
            .convertEthToTokenLP{value: ethAmount}(address(MeadToken));
    ...
    /// @notice Unstakes the desired amount of Mead-BNB-LP tokens from the pool, then converts it into the specified token
    /// @dev Executes claimRewards before the unstaking to get a clean state for the roundMask and the rewards
    /// @param token The address of the token into which the Mead-BNB-LP tokens will be converted
    /// @param amount The desired amount to unstake for an investor in Mead-BNB-LP tokens
    ...
    /// @dev The round mask is updated in the first place to update the current rewards for the investor. The rewards in mead tokens are then minted and transferred to the investor
    function claimRewards() public whenNotPaused {
        ...
        if (rewardsToTransfer > 0) {
            MeadToken.claimTokens(rewardsToTransfer);
