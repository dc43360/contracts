...
/// @notice This contract keeps track of the referral balances and their mead rewards. It uses the TokenA-TokenB-LP token from the referral recipient to split up the mead rewards for the referral giver using EIP-1973.
/// @dev This contract is intended to be called from a Rewarder contract like the Dwarf contract which wants to keep track of the referrals. It is important to note that the Dwarf contract needs to keep track of the deposit of the referral recipient as this contract only considers the sum of all the deposits from referral recipients for a given referral giver.
    ...
    PausableUpgradeable
{
    ...
    /// @param _rewardedAmount The amount in mead that was rewarded to the contract
        ...
        onlyRole(REWARDER_ROLE)
    {
        ...
        IERC20Upgradeable(address(MeadToken)).safeTransferFrom(
            msg.sender,
            address(this),
            _rewardedAmount
        );
