...
import "./Interfaces/IMead.sol";
import "./Interfaces/IStakingPool.sol";
import "@openzeppelin/contracts-upgradeable/security/PausableUpgradeable.sol";

/// @title The mead staking pool
/// @notice The mead staking pool allows to stake meads and get lp and mead rewards. Lp tokens are rewarded through rewards from the liquidity pools (see tokenflow)
/// @dev The share of lp and mead rewards is done with roundmasks according to EIP-1973
    ...
    PausableUpgradeable
{
    ...
    IMead public StakedToken;
    IERC20Upgradeable public LPToken;

    uint256 private meadRoundMask;
    uint256 private lpRoundMask;
    uint256 private meadMintRoundMask;
    uint256 private lastMeadMintRoundMaskUpdateBlock;
    ...
    uint256 public totalMeadClaimed;
    ...
    function initialize(
        ...
        StakedToken = IMead(tokenAddress);
    ...
    /// @notice Stakes the desired amount of mead into the staking pool
    ...
    function stake(uint256 amount) external override whenNotPaused {
        // if first stake initialize lastMeadMintRoundMaskUpdateBlock
        if (lastMeadMintRoundMaskUpdateBlock == 0) {
            lastMeadMintRoundMaskUpdateBlock = block.number;
        }
        ...
        if (stakerAmounts[msg.sender].meadMintMask == 0)
            stakerAmounts[msg.sender].meadMintMask = meadMintRoundMask;
        ...
        stakerAmounts[msg.sender].meadMask = meadRoundMask;

        emit Stake(msg.sender, amount);
    }

    /// @notice Unstake the desired amount of mead from the staking pool
    ...
    function unstake(uint256 amount) external override whenNotPaused {
        ...
        stakerAmounts[msg.sender].stakedAmount = currentBalance - amount;
        stakerAmounts[msg.sender].meadMask = meadRoundMask;
        stakerAmounts[msg.sender].claimedMead += amount;
    ...
    /// @dev Returns the staked amount and all the mead rewards together
    /// @param staker The staker address whos balance is requested
    /// @return The staked amount in mead
    function balanceOf(address staker) public view override returns (uint256) {
        if (stakerAmounts[staker].meadMask == 0) return 0;

        return
            stakerAmounts[staker].stakedAmount +
            ((meadRoundMask - stakerAmounts[staker].meadMask) *
                stakerAmounts[staker].stakedAmount) /
            DECIMAL_OFFSET;
    }

    /// @notice Rewards the staking pool with mead
    /// @dev The round mask is increased according to the reward
    /// @param amount The amount to be rewarded
    function rewardMead(uint256 amount) external override whenNotPaused {
        ...
        meadRoundMask += (DECIMAL_OFFSET * amount) / totalStaked;
    }
    ...
    /// @notice Updates the additional Mead Minting round mask
    ...
    function updateAdditionalMintRoundMask() public override whenNotPaused {
        ...
        uint256 totalPendingRewards = getMeadMintRewardsInRange(
            lastMeadMintRoundMaskUpdateBlock,
            block.number
        );
    ...
    function updateAdditionalMintRewardMask() public whenNotPaused {
        ...
        uint256 pendingMeadRewards = getPendingMeadRewards();

        stakerAmounts[msg.sender].pendingMeadMint = pendingMeadRewards;
        stakerAmounts[msg.sender].meadMintMask = meadMintRoundMask;
    }
    ...
    /// @notice Returns te pending amount of minted Mead rewards
    /// @dev The result is based on the current round mask, as well as the change in the round mask since the last update
    /// @return Pending rewards
    function getPendingMeadRewards() public view override returns (uint256) {
        if (stakerAmounts[msg.sender].meadMintMask == 0) return 0;

        uint256 currentRoundMask = meadMintRoundMask;
        ...
        return
            stakerAmounts[msg.sender].pendingMeadMint +
            ((currentRoundMask - stakerAmounts[msg.sender].meadMintMask) *
                stakerAmounts[msg.sender].stakedAmount) /
            DECIMAL_OFFSET;
    }

    /// @notice Withdraws LP tokens to remove liquidity from pancakeswap and withdraws additional mead rewards
    /// @dev Uses the pancakeswap router to remove liquidity with the desired LP amount. The staked token and the corresponding bnb are transferred to the account. In addition a reward in mead token is calculated and minted for the account
    /// @param amount The desired lp amount which should be used to remove liquidity from pancakeswap
    /// @param additionalMeadAmount The desired additional mead amount to be claimed
    /// @param to The Account the staked token, the bnb and the additional mead reward is sent to
        ...
        returns (uint256 stakedTokenOut, uint256 bnbOut)
    {
        ...
        if (additionalMeadAmount > 0) {
            require(
                stakerAmounts[msg.sender].pendingMeadMint >=
                    additionalMeadAmount,
                "Requested additionalMeadAmount too large"
            );

            stakerAmounts[msg.sender].pendingMeadMint -= additionalMeadAmount;
            stakerAmounts[msg.sender].claimedMeadMint += additionalMeadAmount;

            StakedToken.claimTokens(additionalMeadAmount);
            IERC20Upgradeable(address(StakedToken)).safeTransfer(
                to,
                additionalMeadAmount
            );
        ...
        totalMeadClaimed += removedStakedToken + additionalMeadAmount;
        totalBnbClaimed += removedBnb;

        emit ClaimRewards(
            msg.sender,
            removedStakedToken + additionalMeadAmount,
            removedBnb
        );
        return (removedStakedToken + additionalMeadAmount, removedBnb);
    }
    ...
    /// @notice Sets the mead minting transition times and amounts
