...
/// @title Base config for dwarf contract
/// @notice This contract contains all external addresses and dependencies for the dwarf contract. It also approves dependent contracts to spend tokens on behalf of dwarf.sol
/// @dev The contract dwarf.sol inherits this contract to have all dependencies available. This contract is always inherited and never deployed alone
    ...
    PausableUpgradeable
{
    ...
    IERC20Upgradeable public MeadBnbLpToken;
    ...
    function __BaseConfig_init(
        ...
        MeadBnbLpToken = IERC20Upgradeable(_MeadBnbLpTokenAddress);
        ...
        IERC20Upgradeable(address(MeadToken)).safeApprove(
            address(StakingPool),
            type(uint256).max
        );
        IERC20Upgradeable(address(MeadToken)).safeApprove(
        ...
        IERC20Upgradeable(address(MeadBnbLpToken)).safeApprove(
            address(StakingPool),
            type(uint256).max
        );
    }
