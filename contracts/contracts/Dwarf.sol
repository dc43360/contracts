...
import "./DEX.sol";
import "./Strategy/DwarfStrategy.sol";
    ...
    IDwarf
{
    ...
    function initialize(
        ...
        address _MeadBnbLpTokenAddress,
    ...
    function withdrawAll(
        ...
        } else if (userStrategy[msg.sender] == Strategy.DWARF) {
            currentDeposits = getDwarfStrategyBalance();
        } else {
    ...
    function _withdraw(uint256 amount) internal returns (uint256) {
        if (userStrategy[msg.sender] == Strategy.STANDARD) {
            ...
            standardStrategyClaimMead();
        } else if (userStrategy[msg.sender] == Strategy.DWARF) {
            dwarfStrategyWithdraw(amount);
            dwarfStrategyClaimMead();
            dwarfStrategyClaimLP();
        } else {
    ...
    function changeStrategy(
        ...
        if (userStrategy[msg.sender] == Strategy.STANDARD) {
            ...
            currentDeposits = getDwarfStrategyBalance();
            if (currentDeposits > 0) {
                dwarfStrategyWithdraw(currentDeposits);
                dwarfStrategyClaimMead();
                dwarfStrategyClaimLP();
            }
        } else {
        ...
        if (currentDeposits > 0) {
            ...
            else if (toStrategy == Strategy.DWARF)
                dwarfStrategyDeposit(currentDeposits);
    ...
    /// @return dwarfBnb the dwarf BNB reward
    ...
    {
        // update average mead bnb price
        AveragePriceOracle.updateMeadEthPrice();
        ...
        uint256 dwarfShare = 0;
        if (totalDeposits != 0) {
            standardShare =
                (bnbAmount * standardStrategyDeposits) /
                totalDeposits;
            dwarfShare = (bnbAmount * dwarfStrategyDeposits) / totalDeposits;
        }
        uint256 stablecoinShare = bnbAmount - standardShare - dwarfShare;

        if (standardShare > 100) stakeStandardRewards(standardShare);
        if (dwarfShare > 100) stakeDwarfRewards(dwarfShare);
        if (stablecoinShare > 100) stakeStablecoinRewards(stablecoinShare);

        if (bnbAmount > 100 && totalDeposits != 0) {
            // Get the price of Mead relative to BNB
            uint256 ghnyBnbPrice = AveragePriceOracle.getAverageMeadForOneEth();
            ...
            // Mead (based on Mead-BNB price) is minted
            uint256 mintedMead = mintTokens(referralReward, ghnyBnbPrice);
            // referral contract is rewarded with the minted mead
            Referral.referralUpdateRewards(mintedMead);
        }
        ...
        return (bnbAmount, standardShare, dwarfShare, stablecoinShare);
    }
    ...
    function stakeStandardRewards(uint256 bnbReward) internal {
        ...
        // If Mead price too low, use buyback strategy
        if (ghnyBnbPrice > beeEfficiencyLevel) {
            // 24% of the BNB is used to buy Mead from the DEX
            uint256 meadBuybackShare = (bnbReward * 24) / 100;
            uint256 meadBuybackAmount = DEX.convertEthToToken{
                value: meadBuybackShare
            }(address(MeadToken));

            // 6% of the equivalent amount of Mead (based on Mead-BNB price) is minted
            uint256 mintedMead = mintTokens(
                (bnbReward * 6) / 100,
                beeEfficiencyLevel
            );

            // The purchased and minted Mead is rewarded to the Standard strategy participants
            standardStrategyRewardMead(meadBuybackAmount + mintedMead);

            // The remaining 6% is transferred to the devs
            _transferEth(
                DevTeam,
                bnbReward - tokenPairLpShare - meadBuybackShare
            );
        } else {
            // If Mead price is high, 24% is converted into Mead-BNB LP
            uint256 meadBnbLpShare = (bnbReward * 24) / 100;
            (uint256 meadBnbLpAmount, , ) = DEX.convertEthToTokenLP{
                value: meadBnbLpShare
            }(address(MeadToken));

            // That Mead-BNB LP is sent as reward to the Staking Pool
            StakingPool.rewardLP(meadBnbLpAmount);

            // 30% of the equivalent amount of Mead (based on Mead-BNB price) is minted
            uint256 mintedMead = mintTokens(
                (bnbReward * 30) / 100,
                beeEfficiencyLevel
            );

            // The minted Mead is rewarded to the Standard strategy participants
            standardStrategyRewardMead(mintedMead);

            // The remaining 6% of BNB is transferred to the devs
            _transferEth(
                DevTeam,
                bnbReward - tokenPairLpShare - meadBnbLpShare
            );
        }
    }

    /// @notice Stakes the rewards for the dwarf strategy
    /// @param bnbReward The pending bnb reward to be restaked
    function stakeDwarfRewards(uint256 bnbReward) internal {
        // Get the price of Mead relative to BNB
        uint256 ghnyBnbPrice = AveragePriceOracle.getAverageMeadForOneEth();

        // If Mead price too low, use buyback strategy
        if (ghnyBnbPrice > beeEfficiencyLevel) {
            // 94% (70% + 24%) of the BNB is used to buy Mead from the DEX
            uint256 meadBuybackShare = (bnbReward * (70 + 24)) / 100;
            uint256 meadBuybackAmount = DEX.convertEthToToken{
                value: meadBuybackShare
            }(address(MeadToken));
            ...
            // The purchased and minted Mead is staked
            dwarfStrategyStakeMead(meadBuybackAmount + mintedMead);

            // The remaining 6% of BNB is transferred to the devs
            _transferEth(DevTeam, bnbReward - meadBuybackShare);
        } else {
            // If Mead price is high, 70% of the BNB is used to buy Mead from the DEX
            uint256 meadBuybackShare = (bnbReward * 70) / 100;
            uint256 meadBuybackAmount = DEX.convertEthToToken{
                value: meadBuybackShare
            }(address(MeadToken));

            // 24% of the BNB is converted into Mead-BNB LP
            uint256 meadBnbLpShare = (bnbReward * 24) / 100;
            (uint256 meadBnbLpAmount, , ) = DEX.convertEthToTokenLP{
                value: meadBnbLpShare
            }(address(MeadToken));
            // The Mead-BNB LP is provided as reward to the Staking Pool
            StakingPool.rewardLP(meadBnbLpAmount);
            ...
            _transferEth(
                DevTeam,
                bnbReward - meadBuybackShare - meadBnbLpShare
            );
        }
    }
    ...
    /// @param _share The share that should be minted in mead
    /// @param _beeEfficiencyLevel The bee efficiency level to be uset to convert bnb shares into mead amounts
    /// @return tokens The amount minted in mead tokens
    ...
    /// @return earnedMead - The amount of Mead tokens earned
    /// @return earnedBnb - The amount of BNB earned
    /// @return stakedMead - The amount of Mead tokens staked in the Staking Pool
        ...
        )
    {
        ...
        if (currentStrategy == Strategy.DWARF) {
            deposited = getDwarfStrategyBalance();
            balance = deposited;
            totalReinvested = 0;
            (earnedMead, earnedBnb) = dwarfStrategyClaimLP();
            stakedMead = getDwarfStrategyStakedMead();
            ...
            earnedMead = getStandardStrategyMeadRewards();
            earnedBnb = 0;
            stakedMead = 0;
